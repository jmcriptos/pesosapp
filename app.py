...
def generar_pdf_etiquetas(datos, cantidad):
    """
    Genera un PDF con etiquetas 4x2 (una por página) para etiquetas de vencimiento.
    Usa el mismo formato que las etiquetas de pedidos pero sin cliente ni peso.
    """
    output = BytesIO()

    # ========= CONFIGURACIÓN DE PÁGINA 4" x 2" =========
    PAGE_W = 4 * inch
    PAGE_H = 2 * inch

    c = canvas.Canvas(output, pagesize=(PAGE_W, PAGE_H))

    # ========= DISEÑO / RETÍCULA (igual que etiquetas de pedidos) =========
    M = 8  # Margen interno
    ...
    # Información superior (SIN Cliente, ajustada)
    Y_LOT    = PAGE_H - M - 0.22 * inch
    Y_MFG    = Y_LOT    - 0.18 * inch
    Y_EXP    = Y_MFG    - 0.18 * inch
    Y_KEEP   = Y_EXP    - 0.18 * inch

    # Separador (SIN Net Weight, así que el separador puede estar más arriba)
    SEP_Y    = M + 0.33 * inch

    # Área del producto (más espacio vertical ya que no hay Net Weight)
    PROD_Y_MIN = M + 0.06 * inch
    PROD_Y_MAX = SEP_Y - 0.05 * inch

    logo_path = os.path.join(basedir, 'static', 'logo_etiquetas.png')

    # ========= FUNCIÓN HELPER PARA TEXTO CENTRADO (reutilizada) =========
    def draw_center_wrap_text(canvas_obj, text, center_x, y_bottom, y_top, max_width,
                              font_name="Helvetica-Bold", max_font=19.2, min_font=12, line_gap=2):
        """
        Dibuja 'text' centrado, 1–2 líneas, auto-escala para caber en ancho y alto.
    ...
    for i in range(cantidad):
        ...
        # Valores
        c.setFont("Helvetica", 9.5)
        c.drawString(VAL_X, Y_LOT,    datos.get('lote', ''))
        c.drawString(VAL_X, Y_MFG,    datos.get('fecha_fabricacion', ''))
        c.drawString(VAL_X, Y_EXP,    datos.get('fecha_expiracion', ''))
        ...
        # Producto (1-2 líneas, centrado, con más espacio vertical)
        max_text_width = PAGE_W - (2 * M)
        draw_center_wrap_text(
            c,
