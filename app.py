...
def etiquetas_vencimiento():
    """
    Genera etiquetas de vencimiento en formato 4x2 (una por página).
    Similar a las etiquetas de pedidos pero SIN cliente ni peso.
    """
    if request.method == 'POST':
        try:
            producto_id = request.form['producto_id']
            ...
            # Calcular fecha de expiración (365 días por defecto)
            fecha_fabricacion_date = datetime.strptime(fecha_fabricacion, '%Y-%m-%d')
            fecha_expiracion = fecha_fabricacion_date + timedelta(days=365)
            ...
            # Generar PDF
            return generar_pdf_etiquetas_vencimiento(
                producto=producto,
                lote=lote,
                fecha_fab=fecha_fabricacion_date.strftime('%Y-%m-%d'),
                fecha_exp=fecha_expiracion.strftime('%Y-%m-%d'),
                cantidad=cantidad_etiquetas
            )
        except Exception as e:
            flash(f"Error generando etiquetas: {str(e)}", "danger")
            return redirect(url_for('etiquetas_vencimiento'))

    productos = Producto.query.all()
    return render_template('form_generar_etiquetas.html', productos=productos)

def generar_pdf_etiquetas_vencimiento(producto, lote, fecha_fab, fecha_exp, cantidad):
    """
    Genera PDF con etiquetas 4x2 para vencimiento (sin cliente ni peso).
    Formato idéntico a las etiquetas de pedidos.
    """
    # --------- PDF 4" x 2" (una etiqueta por página) ----------
    PAGE_W = 4 * inch
    PAGE_H = 2 * inch
    ...
    # Información superior (SIN CLIENT, solo LOT, MFG, EXP, KEEP)
    Y_LOT    = PAGE_H - M - 0.22 * inch
    Y_MFG    = Y_LOT    - 0.20 * inch
    Y_EXP    = Y_MFG    - 0.20 * inch
    Y_KEEP   = Y_EXP    - 0.20 * inch

    # Separador - más abajo ya que no hay Net Weight
    SEP_Y    = M + 0.40 * inch

    # Área del producto
    PROD_Y_MIN = M + 0.06 * inch
    PROD_Y_MAX = M + 0.33 * inch
    ...
    # ========= DIBUJO DE UNA ETIQUETA =========
    def dibujar_etiqueta(prod, temp, lote_val, f_fab, f_exp):
        # LOGO
        if os.path.exists(logo_path):
            c.drawImage(logo_path, LOGO_X, LOGO_Y, width=LOGO_W, height=LOGO_H,
                        preserveAspectRatio=True, mask='auto')

        # Labels (derecha) - SIN CLIENT
        c.setFont("Helvetica-Bold", 9.5)
        c.drawRightString(LBL_XR, Y_LOT,    "Lot:")
        c.drawRightString(LBL_XR, Y_MFG,    "Manufactured:")
        c.drawRightString(LBL_XR, Y_EXP,    "Expiration:")
        ...
        # Valores
        c.setFont("Helvetica", 9.5)
        c.drawString(VAL_X, Y_LOT,    lote_val or "")
        c.drawString(VAL_X, Y_MFG,    f_fab or "")
        c.drawString(VAL_X, Y_EXP,    f_exp or "")
    ...
    prod_nombre = producto.nombre if producto else "N/A"
    temp = getattr(producto, "temperatura", None) or ""

    for _ in range(cantidad):
        dibujar_etiqueta(prod_nombre, temp, lote, fecha_fab, fecha_exp)

    c.save()
    output.seek(0)

    # --------- Nombre del archivo ----------
    nombre_producto = producto.nombre.replace(" ", "_").replace("/", "-") if producto else "producto"
    filename = f"etiquetas_vencimiento_{nombre_producto}_{lote}.pdf"
