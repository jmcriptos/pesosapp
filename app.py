...
def generar_pdf_etiquetas(datos, cantidad):
    """
    Genera etiquetas de vencimiento con el mismo formato que las etiquetas de pedidos
    pero sin incluir cliente y peso. Formato 4" x 2" (una etiqueta por página).
    """
    output = BytesIO()
    PAGE_W = 4 * inch
    PAGE_H = 2 * inch
    ...
    # Información superior (COMPACTADA) - sin Client
    Y_LOT    = PAGE_H - M - 0.22 * inch
    Y_MFG    = Y_LOT    - 0.18 * inch
    Y_EXP    = Y_MFG    - 0.18 * inch
    Y_KEEP   = Y_EXP    - 0.18 * inch

    # Separador (sin Net Weight, más arriba)
    SEP_Y    = M + 0.46 * inch
